...
function LaundryApp() {
  ...
  /* Confirmation dialog for destructive actions.
     requireText makes the user type a word (e.g. RESET) before the button enables. */
  const [confirmBox, setConfirmBox] = useState(null); // { title, body, confirmLabel, danger, requireText, onConfirm }
  const [confirmText, setConfirmText] = useState("");
  const askConfirm = (cfg) => { setConfirmText(""); setConfirmBox(cfg); };
  ...
  const [importPreview, setImportPreview] = useState(null); // { rows, updated, added, skipped, fileName }
  const parsePriceFile = (file) => {
    if (typeof XLSX === "undefined") { flash("Excel support didn't load — check your internet connection and reload the app."); return; }
    const reader = new FileReader();
    reader.onload = () => {
      try {
        ...
        if (!col) { flash("Couldn't find the header row — the file needs columns: Service Code, Item, Unit, Price (N$)."); return; }
        ...
        if (!rows.length) { flash("No usable rows found — check that Item and Price are filled in."); return; }
        let updated = 0, added = 0;
        const classified = rows.map((row) => {
          const match = services.find((s) =>
            (row.code && String(s.code || s.id).toLowerCase() === row.code.toLowerCase()) ||
            s.name.toLowerCase() === row.name.toLowerCase());
          if (match) { updated++; return { ...row, action: "update", targetId: match.id }; }
          added++; return { ...row, action: "add" };
        });
        setImportPreview({ rows: classified, updated, added, skipped, fileName: file.name });
      } catch (e) {
      ...
      importPreview.rows.forEach((row) => {
        if (row.action === "update") {
          next = next.map((s) => s.id === row.targetId
            ? { ...s, name: row.name, unit: row.unit, price: row.price, code: row.code || s.code, cat: row.cat || s.cat }
            : s);
        } else {
          next.push({ id: newKey(), code: row.code || undefined, name: row.name, unit: row.unit, price: row.price, icon: "🧼", cat: row.cat || "Garments" });
        }
      });
      return next;
    });
  ...
  const exportPriceList = () => {
    ...
    ws["!cols"] = [{ wch: 12 }, { wch: 28 }, { wch: 10 }, { wch: 10 }, { wch: 12 }];
  ...
  const [custHi, setCustHi] = useState(-1); // keyboard-highlighted result
  ...
  /* Everything blocking order creation, listed for the inline checklist
     (recomputed each render so problems disappear as staff fix them). */
  const orderProblems = () => {
    const probs = [];
    if (cartLines.length === 0) probs.push("Add at least one item.");
    if (!customers.find((c) => c.id === sel.customerId)) {
      if (!sel.newMode) probs.push("Search for the customer above, or tap “Register new customer”.");
      else {
        if (!sel.name.trim()) probs.push("Enter the new customer's name.");
        if (!sel.phone.trim()) probs.push("Enter the new customer's phone number.");
        else if (!phoneLooksValid(sel.phone)) probs.push("Check the phone number — just the local number, e.g. 0813886676.");
      }
    }
    return probs;
  };
  ...
  const createOrder = (custOverride) => {
    ...
    if (probs.length > 0) { setTriedCreate(true); flash(probs[0]); return; }
    let cust = custOverride || customers.find((c) => c.id === sel.customerId);
    if (!cust) {
      /* New registration: guard against a second record on the same number. */
      ...
      if (dupe) {
        askConfirm({
          ...
          onConfirm: () => { pickCustomer(dupe); createOrder(dupe); },
        });
        return;
      }
      cust = {
  ...
  /* Undo a mis-tapped status: back one step, on the record.
     Staff can step back within 15 minutes of the last change; the owner always can. */
  const stepBackStatus = (orderId) => {
    setOrders((os) => os.map((o) => {
      ...
      return { ...o, status: prev, statusHistory: [...o.statusHistory, { status: prev, ts: Date.now(), by: currentStaff, back: true }] };
    }));
  ...
  const [custDraft, setCustDraft] = useState({ name: "", phone: "", address: "" });
  ...
  const addCustomer = () => {
    ...
    if (!name || !custDraft.phone.trim()) { flash("Enter the customer's name and phone."); return; }
    if (!phoneLooksValid(custDraft.phone)) { flash("Check the phone number — just the local number, e.g. 0813886676."); return; }
    const phone = displayPhone(custDraft.phone);
    const dup = customers.find((c) => String(c.phone).replace(/\D/g, "") === phone.replace(/\D/g, ""));
    if (dup) { flash(`That phone number is already registered to ${dup.name}.`); return; }
    setCustomers((cs) => [...cs, { id: nextCustomerId(cs), name, phone, address: custDraft.address.trim() || "Oshikuku", ts: Date.now() }]);
    setCustDraft({ name: "", phone: "", address: "" });
  ...
  const report = useMemo(() => {
    ...
    /* Revenue counts payments on live orders only — a cancelled order's
       payments are excluded (reverse them if the customer was refunded). */
    const collected = [];
    live.forEach((o) => o.payments.forEach((p) => { if ((p.ts || 0) >= start) collected.push(p); }));
  ...
  /* ---------- idle auto-lock (5 minutes without a touch or key) ---------- */
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("pointerdown", bump); window.removeEventListener("keydown", bump); clearInterval(iv); };
  }, [unlocked]);
      ...
      <div className="app">
        ...
        <div className="login-wrap">
          <div className="login-card">
            <span className="brand-drop big" aria-hidden="true">💧</span>
            <h1 className="login-title">{settings.shopName}</h1>
            <p className="muted center-text">Enter your personal PIN to sign in</p>
            <div className="pin-row">
              <input type="password" value={pinInput} inputMode="numeric" maxLength={6} placeholder="••••" className="pin-field" autoFocus
  ...
  return (
    <div className="app">
      ...
      {/* ---------- print overlay ---------- */}
      {printOrder && (
        <Modal label={printing.type === "slip" ? `Laundry slip ${printOrder.id}` : `Receipt for ${printOrder.id}`}
          onClose={() => setPrinting(null)} overlayClassName="print-overlay" className="print-modal-body">
      ...
      {confirmBox && (
        <Modal label={confirmBox.title} onClose={() => setConfirmBox(null)} className="edit-panel confirm-panel">
            <h2>{confirmBox.title}</h2>
            <p className="muted">{confirmBox.body}</p>
            {confirmBox.requireText && (
              <input value={confirmText} placeholder={`Type ${confirmBox.requireText} to confirm`} autoFocus
                onChange={(e) => setConfirmText(e.target.value)} />
            )}
            <div className="edit-actions">
              <button className={"btn-main" + (confirmBox.danger ? " danger-bg" : "")}
              ...
              <button className="btn-ghost" onClick={() => setConfirmBox(null)}>Go back</button>
            </div>
        </Modal>
      )}
      ...
      {/* ---------- edit order overlay ---------- */}
      {edit && (
        <Modal label={`Edit order ${edit.orderId}`} onClose={() => setEdit(null)} className="edit-panel">
            ...
            <div className="edit-adders">
              <div className="edit-add">
                <select value={edit.addId} aria-label="Add an item from the price list" onChange={(e) => setEdit({ ...edit, addId: e.target.value })}>
              ...
              <div className="edit-add">
                <input value={edit.customDraft.name} placeholder="Custom item name" aria-label="Custom item name"
                  onChange={(e) => setEdit({ ...edit, customDraft: { ...edit.customDraft, name: e.target.value } })} />
                <input type="number" min="0" value={edit.customDraft.price} placeholder="Price" aria-label="Custom item price" className="narrow"
      ...
      <div className={"shell layout" + (printOrder || edit || confirmBox ? " shell-dim" : "")}>
        {/* ---------- sidebar ---------- */}
        <aside className="sidebar no-print">
          <div className="sb-brand">
            <span className="sb-logo" aria-hidden="true">💧</span>
            <div>
          ...
          <nav className="sb-nav">
            <button className={tab === "dash" ? "on" : ""} aria-current={tab === "dash" ? "page" : undefined} onClick={() => setTab("dash")}><i aria-hidden="true">🏠</i> Dashboard</button>
            <button className={tab === "new" ? "on" : ""} aria-current={tab === "new" ? "page" : undefined} onClick={() => setTab("new")}><i aria-hidden="true">➕</i> New order</button>
            <button className={tab === "orders" ? "on" : ""} aria-current={tab === "orders" ? "page" : undefined} onClick={() => setTab("orders")}><i aria-hidden="true">🧺</i> Orders{openCount ? <em className="sb-badge">{openCount}</em> : null}</button>
            <button className={tab === "customers" ? "on" : ""} aria-current={tab === "customers" ? "page" : undefined} onClick={() => setTab("customers")}><i aria-hidden="true">👥</i> Customers</button>
            {isAdmin && <button className={tab === "reports" ? "on" : ""} aria-current={tab === "reports" ? "page" : undefined} onClick={() => setTab("reports")}><i aria-hidden="true">📊</i> Reports</button>}
            {isAdmin && <button className={tab === "prices" ? "on" : ""} aria-current={tab === "prices" ? "page" : undefined} onClick={() => setTab("prices")}><i aria-hidden="true">🏷️</i> Price list</button>}
            {isAdmin && <button className={tab === "team" ? "on" : ""} aria-current={tab === "team" ? "page" : undefined} onClick={() => setTab("team")}><i aria-hidden="true">👤</i> Team</button>}
          ...
          <div className="sb-foot">
            ...
            <button className="btn-lock" onClick={() => { setUnlocked(false); setCurrentStaff(""); }}><span aria-hidden="true">🔒</span> Lock</button>
          </div>
        </aside>
        ...
        <div className="main-col">
          ...
          <main className="wrap">
            ...
            {isAdmin && defaultPinsActive && tab !== "team" && (
              <div className="warn-banner no-print">
                ⚠️ The demo PINs (1234 / 5678) are still active — anyone who knows them can sign in.
                <button className="ft-link" onClick={() => setTab("team")}>Change them in the Team tab</button>
              </div>
            )}
          ...
          {tab === "dash" && (
            <section>
              ...
              <div className="report-grid">
                ...
                {dash.owing.length > 0 && (
                  <div className="panel">
                    <h2>Collected but still owing</h2>
                    <p className="muted">Items already released with a balance owing — follow up until it's settled.</p>
                    {dash.owing.map((o) => (
                      <div key={o.id} className="dash-row">
                        <div>
                          <b>{o.id}</b> · {o.customerName}
          ...
          {tab === "new" && (
            <section className="order-grid">
              <div className="panel">
                  ...
                  <div className="cust-search">
                    <div className="cust-combo">
                      <input value={custQuery} placeholder="Select or search customer…"
                        role="combobox" aria-expanded={custFocus || !!custQuery.trim()} aria-controls="cust-listbox"
                        aria-label="Search customers by name or phone" aria-autocomplete="list"
                        ...
                        onChange={(e) => { setCustQuery(e.target.value); setCustFocus(true); setCustHi(-1); }}
                        onKeyDown={(e) => {
                          if (e.key === "ArrowDown") { e.preventDefault(); setCustFocus(true); setCustHi((h) => Math.min(h + 1, custResults.length - 1)); }
                          else if (e.key === "ArrowUp") { e.preventDefault(); setCustHi((h) => Math.max(h - 1, 0)); }
                          else if (e.key === "Enter" && custHi >= 0 && custResults[custHi]) { e.preventDefault(); pickCustomer(custResults[custHi]); setCustFocus(false); setCustHi(-1); }
                          else if (e.key === "Escape") { setCustFocus(false); setCustHi(-1); }
                        }} />
                      <span className="combo-arrow" aria-hidden="true">▾</span>
                    </div>
                    {(custFocus || custQuery.trim()) && (
                      <div className="cust-results" id="cust-listbox" role="listbox" onMouseDown={(e) => e.preventDefault()}>
                        {custResults.map((c, i) => (
                          <button key={c.id} id={`cust-opt-${i}`} role="option" aria-selected={i === custHi}
                            className={"cust-result" + (i === custHi ? " hi" : "")}
                            onClick={() => { pickCustomer(c); setCustFocus(false); setCustHi(-1); }}>
                ...
                <div className="edit-add">
                  <input value={customDraft.name} placeholder="e.g. Wedding gown" aria-label="Custom item name"
                    onChange={(e) => setCustomDraft({ ...customDraft, name: e.target.value })} />
                  <input type="number" min="0" value={customDraft.price} placeholder="Price" aria-label="Custom item price" className="narrow"
                ...
                <div className="form two-col">
                  ...
                  <label>Payment now (optional)
                    ...
                    {total > 0 && (
                      <span className="amt-chips">
                        <button type="button" className="amt-chip" onClick={() => setPayNow({ ...payNow, amount: String(total) })}>Full {fmt(total)}</button>
                        <button type="button" className="amt-chip" onClick={() => setPayNow({ ...payNow, amount: String(Math.ceil(total / 2)) })}>Half {fmt(Math.ceil(total / 2))}</button>
                      </span>
                    )}
                    {parseFloat(payNow.amount) > total && (
                      <span className="overpay-note">That's more than the total ({fmt(total)}) — is that right?</span>
                    )}
                  </label>
              ...
              <aside className="ticket">
                ...
                {triedCreate && orderProblems().length > 0 && (
                  <div className="err-box" role="alert">
                    <b>Before creating this order:</b>
                    <ul>{orderProblems().map((p, i) => <li key={i}>{p}</li>)}</ul>
                  </div>
                )}
          ...
          {tab === "orders" && (
            <section>
              <div className="order-toolbar no-print">
                <input className="search" value={search} aria-label="Search orders by name, phone or order number" placeholder="Search name, phone or order no…"
                  onChange={(e) => setSearch(e.target.value)} />
                <div className="chip-row">
                  {[["open", "Open"], ["unpaid", "Unpaid"], ["done", "Completed"], ["all", "All"]].map(([k, l]) => (
                    <button key={k} className={"chip-btn" + (filter === k ? " on" : "")} aria-pressed={filter === k} onClick={() => setFilter(k)}>{l}</button>
                  ))}
                </div>
              </div>
                  ...
                  <div key={o.id} className={"panel admin-order" + (o.cancelled ? " order-cancelled" : "")}>
                    ...
                    {!o.cancelled && !isCancelling && (
                      <div className="ao-actions">
                        {idx < STATUSES.length - 1 ? (
                          <button className="btn-main small" onClick={() => {
                            const bal = balanceOf(o);
                          ...
                          }}>Mark “{STATUSES[idx + 1]}”</button>
                        ) : <span className="chip pay-paid">Completed</span>}
                        {canStepBack(o) && (
                          <button className="btn-ghost small" title="Undo the last status change" onClick={() => stepBackStatus(o.id)}>↩ Step back</button>
                        )}
                        <button className="btn-ghost small" onClick={() => setPrinting({ type: "slip", orderId: o.id })}><span aria-hidden="true">🧾</span> Slip</button>
                        {o.payments.length > 0 && (
                        ...
                        {o.status !== DONE_STATUS && (
                          <>
                            <button className="btn-ghost small" onClick={() => openEdit(o)}><span aria-hidden="true">✏️</span> Edit</button>
                    ...
                    {isCancelling && (
                      <div className="cancel-form">
                        ...
                        {paidOf(o) > 0 && (
                          <span className="overpay-note">{fmt(paidOf(o))} has been paid on this order. If you refund it, also reverse the payment(s) so the books stay right — cancelled orders don't count in sales.</span>
                        )}
                      </div>
                    )}
                    ...
                    {!o.cancelled && ps !== "paid" && !isCancelling && (
                      <div className="pay-form">
                        ...
                        <button type="button" className="amt-chip" onClick={() => setPayDraft((d) => ({ ...d, [o.id]: { ...draft, amount: String(balanceOf(o)) } }))}>Full {fmt(balanceOf(o))}</button>
                        <select value={draft.method} aria-label="Payment method"
                        ...
                        {parseFloat(draft.amount) > balanceOf(o) && (
                          <span className="overpay-note">That's more than the balance ({fmt(balanceOf(o))}) — is that right?</span>
                        )}
                      </div>
                    )}
          ...
          {tab === "customers" && (
            <section className="panel">
              <h2>Customers</h2>
              <p className="muted">Register customers here or on the “+ New order” screen during their first visit. Use Edit to fix a wrong name or phone number.</p>
              <div className="svc-cat">Add a customer</div>
              <div className="edit-add">
                <input value={custDraft.name} placeholder="Full name" aria-label="Customer full name"
                  onChange={(e) => setCustDraft({ ...custDraft, name: e.target.value })} />
                <input value={custDraft.phone} placeholder="Phone, e.g. 0813886676" aria-label="Customer phone number" inputMode="tel" className="narrow"
                  onChange={(e) => setCustDraft({ ...custDraft, phone: e.target.value })} />
                <input value={custDraft.address} placeholder="Address (optional)" aria-label="Customer address (optional)"
                  onChange={(e) => setCustDraft({ ...custDraft, address: e.target.value })} />
                <button className="btn-main small" onClick={addCustomer}>Add</button>
              </div>
              <div className="order-toolbar no-print">
                <input className="search" value={custTabQuery} aria-label="Search customers by name, phone or address" placeholder="Search name, phone or address…"
                  ...
                  {filteredCustomers.map((c) => {
                    ...
                    return editing ? (
                      <tr key={c.id} className="cust-edit-row">
                        <td>{c.id}</td>
                        <td><input value={custEdit.name} aria-label="Name" onChange={(e) => setCustEdit({ ...custEdit, name: e.target.value })} /></td>
                        <td><input value={custEdit.phone} aria-label="Phone" onChange={(e) => setCustEdit({ ...custEdit, phone: e.target.value })} /></td>
                        <td><input value={custEdit.address} aria-label="Address" onChange={(e) => setCustEdit({ ...custEdit, address: e.target.value })} /></td>
                        <td colSpan="3">
                          <div className="dash-acts">
            ...
            <section>
              <div className="report-head no-print">
                <div className="chip-row">
                  {[["today", "Today"], ["7", "Last 7 days"], ["30", "Last 30 days"], ["all", "All time"]].map(([k, l]) => (
                    <button key={k} className={"chip-btn" + (period === k ? " on" : "")} aria-pressed={period === k} onClick={() => setPeriod(k)}>{l}</button>
                  ))}
                </div>
              </div>
              ...
              <div className="report-grid">
                <div className="panel">
                  ...
                  {report.cancelled > 0 && <p className="tiny left">{report.cancelled} cancelled order{report.cancelled > 1 ? "s" : ""} in this period — cancelled orders and their payments are excluded from these figures.</p>}
                </div>
          ...
          {tab === "prices" && isAdmin && (
            <section className="panel">
              ...
              <div className="edit-add" style={{ marginBottom: 14 }}>
                <button className="btn-main small" onClick={() => priceFileInput.current && priceFileInput.current.click()}>⬆ Import price list (Excel)</button>
                <button className="btn-ghost small" onClick={exportPriceList}>⬇ Download as Excel</button>
                <input type="file" ref={priceFileInput} accept=".xlsx,.xls,.csv" style={{ display: "none" }}
                  onChange={(e) => { const f = e.target.files && e.target.files[0]; if (f) parsePriceFile(f); e.target.value = ""; }} />
              </div>
              <p className="muted" style={{ marginTop: -6 }}>Excel columns: <strong>Service Code · Item · Unit · Price (N$)</strong> (Category optional). Items are matched by code or name — matches update the price, new ones are added. Tip: download the current list first and use it as your template.</p>
              {importPreview && (
                <div style={{ background: "#F0F6FF", border: "1px solid #BFDBFE", borderRadius: 10, padding: 14, marginBottom: 16 }}>
                  <strong>Import preview — {importPreview.fileName}</strong>
                  <p className="muted" style={{ margin: "6px 0 10px" }}>
                    {importPreview.updated} item(s) will be updated, {importPreview.added} new item(s) added
                    {importPreview.skipped ? `, ${importPreview.skipped} row(s) skipped (missing name or price)` : ""}. Nothing changes until you press Apply.
                  </p>
                  <table className="table">
                    <thead><tr><th>Code</th><th>Item</th><th>Unit</th><th>Price (N$)</th><th>Action</th></tr></thead>
                    <tbody>
                      {importPreview.rows.map((r, i) => (
                        <tr key={i}>
                          ...
                          <td>{r.unit}</td>
                          <td>{r.price}</td>
                          <td><span className={"chip " + (r.action === "add" ? "role-admin" : "role-staff")}>{r.action === "add" ? "New" : "Update"}</span></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                  <div style={{ display: "flex", gap: 10, marginTop: 12 }}>
                    <button className="btn-main small" onClick={applyImport}>Apply import</button>
                    <button className="btn-ghost small" onClick={() => setImportPreview(null)}>Cancel</button>
                  </div>
                </div>
              )}
              <table className="table">
              ...
              <div className="edit-add">
                <input value={svcDraft.name} placeholder="Item name, e.g. School uniform" aria-label="New item name"
                  onChange={(e) => setSvcDraft({ ...svcDraft, name: e.target.value })} />
                <input type="number" min="0" value={svcDraft.price} placeholder="Price" aria-label="New item price" className="narrow"
                  onChange={(e) => setSvcDraft({ ...svcDraft, price: e.target.value })} />
                <select value={svcDraft.unit} aria-label="Unit" className="narrow"
                  onChange={(e) => setSvcDraft({ ...svcDraft, unit: e.target.value })}>
                  <option>per item</option><option>per pair</option><option>per kg</option>
                </select>
                <select value={svcDraft.cat} aria-label="Category" className="narrow"
          ...
          {tab === "team" && isAdmin && (
            <section className="panel">
                  ...
                  {users.map((u) => {
                    ...
                    return editing ? (
                      <tr key={u.id} className="cust-edit-row">
                        ...
                        <td>
                          <div className="dash-acts">
                            <button className="btn-main small" onClick={saveUserEdit}>Save</button>
                            <button className="btn-ghost small" onClick={() => setUserEdit(null)}>Discard</button>
                          </div>
                        </td>
                      </tr>
                    ) : (
              ...
              <div className="edit-add">
                <input value={userDraft.name} placeholder="Name" aria-label="Person's name"
                  onChange={(e) => setUserDraft({ ...userDraft, name: e.target.value })} />
                <input value={userDraft.pin} placeholder="PIN (4–6 digits)" aria-label="PIN, 4 to 6 digits" inputMode="numeric" maxLength={6} className="narrow"
                  onChange={(e) => setUserDraft({ ...userDraft, pin: e.target.value.replace(/\D/g, "") })} />
                <select value={userDraft.role} aria-label="Role" className="narrow"
          ...
          {tab === "settings" && isAdmin && (
            <section className="panel">
              ...
              <div className="form two-col">
                ...
                <label className="express-inline span2">
                  <input type="checkbox" checked={settings.autoBackup !== false}
              ...
              <div className="edit-add">
                <input value={newMethod} placeholder="e.g. EasyWallet" aria-label="New payment method"
              ...
              <div className="edit-add">
                <input value={newSlot} placeholder="e.g. 09:00 – 12:00 (Sunday)" aria-label="New collection time slot"
              ...
              <textarea className="msg-area" rows={4} value={settings.readyMsg} aria-label="Ready-for-collection message template"
              ...
              <textarea className="msg-area" rows={4} value={settings.overdueMsg} aria-label="Overdue reminder message template"
              ...
              <textarea className="msg-area" rows={3} value={settings.thanksMsg} aria-label="Payment thank-you message template"
              ...
              <textarea className="msg-area" rows={3} value={settings.slipFooter} aria-label="Slip footer note"
...
/* Accessible modal shell: dialog semantics, focus trap, Esc to close,
   focus returned to the opener on close. */
function Modal({ label, onClose, className, overlayClassName = "print-overlay no-print", children }) {
  ...
  return (
    <div className={overlayClassName} role="dialog" aria-modal="true" aria-label={label}>
      <div className={className} ref={ref} tabIndex={-1}>{children}</div>
    </div>
  );
}
